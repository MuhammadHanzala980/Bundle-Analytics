...
/**
 * Compute 'after' ISO: FORCE start at 1 Feb 2024 00:00:00 UTC by default.
 * You can override by setting env var WC_FETCH_AFTER to an ISO date string.
 * This ensures we always fetch orders from 2024-02-01 up to now.
 */
async function computeAfterIso() {
  // allow override from env if you want a different start date
  if (process.env.WC_FETCH_AFTER) {
    try {
      const d = new Date(process.env.WC_FETCH_AFTER);
      if (!isNaN(d)) return d.toISOString();
    } catch (e) {
      // fallthrough to default
      console.log("[sync-orders] WC_FETCH_AFTER invalid, falling back to default start date.");
    }
  }

  const start = new Date(Date.UTC(2024, 1, 1, 0, 0, 0)); // 2024-02-01T00:00:00Z
...
export async function POST() {
  ...
  // compute afterISO (FORCED start date: 2024-02-01 or WC_FETCH_AFTER override)
  const afterIso = await computeAfterIso();
  ...
  // If progress exists and lastFetchedIso is at/after our requested afterIso, resume from progress.
  // Otherwise ignore progress and restart from page=1 (to ensure full fetch from 2024-02-01).
  if (progress && progress.lastFetchedIso) {
    try {
      const progDate = new Date(progress.lastFetchedIso);
      const startDate = new Date(afterIso);
      if (!isNaN(progDate) && progDate >= startDate) {
        page = Number(progress.lastPageFetched) || 1;
        skipIndex = Number(progress.lastIndexInPage) || 0;
        if (skipIndex >= PER_PAGE) { page = page + 1; skipIndex = 0; }
        console.log(`[sync-orders] Resuming from saved progress (page=${page}, skipIndex=${skipIndex}, lastFetchedIso=${progress.lastFetchedIso})`);
      } else {
        console.log("[sync-orders] Existing progress is older than requested start date. Restarting full fetch from page=1.");
      }
    } catch (e) {
      console.log("[sync-orders] Failed to interpret progress file — starting from scratch.", e?.message || e);
    }
  } else {
    console.log("[sync-orders] No valid progress found — starting from page=1.");
  }

  console.log(`[sync-orders] START sync. fetching orders after=${afterIso}`);
  console.log(`[sync-orders] Previously saved orders (deduped): ${existingMap.size}`);
